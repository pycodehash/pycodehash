...
class ProcessArguments(ast.NodeVisitor):
    ...
    def visit_arg(self, node: ast.arg):
        print(node.arg, self.expr_scope, node.annotation)
...
class AnnotateScope(GroupSimilarConstructsVisitor):
    ...
    def visit_alias(self, alias_node):
        variable = alias_node.asname or alias_node.name
        self.annotate_intermediate_scope(alias_node, variable, True)
        self.scope.modify(variable)

    def visit_arg(self, arg: ast.arg):
        print('visits arg', arg.arg)
        self.annotate_intermediate_scope(arg, arg.arg, True)
        self.scope.modify(arg.arg)

    def visit_keyword(self, kw: ast.keyword):
        print('visists kw', kw.arg)
        self.annotate_intermediate_scope(kw, kw.arg, True)
        # value is kw.value
        self.scope.modify(kw.arg)
